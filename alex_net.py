...
def inference(images):
    ...
    with tf.name_scope('conv2') as scope:
        ...
        biases = tf.Variable(tf.constan(0.0, shape=[192], dtype=tf.float32), trainable=True, name='biases')
        bias = tf.nn.bias_add(conv, biases)
    ...
    with tf.name_scope('conv4') as scope:
        ...
        biases = tf.Variable(tf.truncated_normal(0.0, shape=[256], dtype=tf.float32), trainable=True, name='biases')
        bias = tf.nn.bias_add(conv, biases)
        conv4 = tf.nn.relu(bias, biases, name=scope)
        parameters = parameters + [kernel, biases]
